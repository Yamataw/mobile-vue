<template>
  ...
  <div class="flex-col">
    <div class="">
      <div class="bg-cyan-700 h-screen ">
        <br>
          ...
          <div>
            ...
            <div class="relative mx-4">
              <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <svg aria-hidden="true" class="w-5 h-5 text-gray-500 dark:text-g  ray-400" fill="none"
                     stroke="currentColor"
              ...
              <input v-on:input='api' type="search" id="default-search"
                     ...
                     placeholder="Paris, Moscou, Venise..." required list="cities">
        ...
        <div
          ...
          <div class="text-3xl font-bold text-white mb-6">{{ meteo_searched.city }}</div>
          <div class="text-md font-bold flex flex-col text-white"><span class="uppercase">{{
              meteo_searched.time
            }}</span>
            <span class="font-normal text-white text-sm"></span></div>
          <img :src=image alt="meteo icon" class="w-48">
          <div class="text-3xl font-bold text-white mb-6">{{ meteo_searched.temperature }}</div>
        </div>
      </div>
    </div>
<BottomNav></BottomNav>
...
export default {
  ...
  methods: {
        ...
        if (value.name === text) {
          ...
          fetch(`https://api.open-meteo.com/v1/forecast?latitude=${value.latitude}&longitude=${value.longitude}&hourly=temperature_2m,relativehumidity_2m&current_weather=true`, requestOptions)
              ...
              .then(result => {
                ...
                this.meteo_searched.time = this.timeString(this.meteo_searched.time)
              })
              .catch(error => console.log('error', error));
    ...
    timeString(string) {
      ...
      string = string.split('-');
      let dayHourAndMinute = string[2].split('T');
      let monthId = parseInt(string[1]) - 1
      console.log(dayHourAndMinute)
      let hourMinute = dayHourAndMinute[1].split(':');
      hourMinute[0] = parseInt(hourMinute[0]) + 1
      let finalString = `${dayHourAndMinute[0]} ${months[monthId]} ${string[0]} à ${hourMinute.join('h')}`;
      return finalString
